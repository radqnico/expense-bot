...
logger = logging.getLogger("bot-spese.quick")
...
async def cmd_expense(update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not chat or not (update.message and update.message.text):
        return
    logger.info("/expense invoked by chat %s", getattr(chat, "id", "?"))
    ...
    try:
        ...
        logger.exception("Insert failed on /expense: %s", e)
...
async def cmd_income(update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not chat or not (update.message and update.message.text):
        return
    logger.info("/income invoked by chat %s", getattr(chat, "id", "?"))
    parts = update.message.text.split(maxsplit=2)
    if len(parts) < 3:
    ...
    try:
        ...
        logger.exception("Insert failed on /income: %s", e)
        await update.message.reply_text(f"Insert failed: {e}")
        return
    await update.message.reply_text(
